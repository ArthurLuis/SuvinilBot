...
Given the user's question, the paint name and recommended color (in Portuguese), respond with **only** a JSON object containing:
...
Do not include code blocks, comments, or any additional fields—only the JSON object.
Use any relevant details from the user’s question to populate the fields.
All fields are required and must be in english.

User question: "{userMessage}"
Recommended paint (in Portuguese): "{tinta}"
Recommended color (in Portuguese): "{corRecomendada}"
`);
